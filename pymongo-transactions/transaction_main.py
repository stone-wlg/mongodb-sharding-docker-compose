...
def book_seat(seats, payments, audit, seat_no, delay_range, session=None):
    ...
    audit.update_one({"audit": "seats"}, {"$inc": {"count": 1}},
...
if __name__ == "__main__":
    ...
    parser.add_argument("--host", default="mongodb://localhost:27017/?retryWrites=false",
                        help="MongoDB URI [default: %(default)s]")
    parser.add_argument("--usetxns", default=True, action="store_true",
                        help="Use transactions [default: %(default)s]")
    ...
    parser.add_argument("--iterations", default=0, type=int,
                        help="Run  N iterations. O means run forever")
    ...
    client = pymongo.MongoClient(host=args.host, connect=False)
    ...
    print("using collection: {}.{}".format(
        seatsdb.name, seats_collection.name))
    print("using collection: {}.{}".format(
        paymentsdb.name, payments_collection.name))
    print("using collection: {}.{}".format(
        auditdb.name, audit_collection.name))
    ...
    doc = client.admin.command(
        {"getParameter": 1, "featureCompatibilityVersion": 1})
    # if doc["featureCompatibilityVersion"]["version"] != "4.0":
    #     print("Your mongod is set to featureCompatibility: {}".format(
    #         doc["featureCompatibilityVersion"]["version"]))
    #     print(
    #         "(This happens if you run mongod 4.0 and point it at data directory created with")
    ...
    if args.randdelay:
        ...
        print("Using a random delay between {} and {}".format(
            args.randdelay[0], args.randdelay[1]))
    else:
